            ...
            <div class="card-body py-4">
                <form action="{{ route('roles-management.update', $role->id) }}" method="POST" id="roleForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="mb-6">
                                ...
                                <small class="text-muted">Role aktif dapat digunakan</small>
                            </div>

                            <div class="mb-6">
                                <label for="level" class="form-label fw-bold">Level Role</label>
                                <input type="number" class="form-control @error('level') is-invalid @enderror"
                                       id="level" name="level" value="{{ old('level', $role->level ?? 1) }}"
                                       min="1" max="10" placeholder="Level role">
                                @error('level')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="text-muted">Level hierarki role (1-10)</small>
                            </div>
                    ...
                    <div class="mb-6">
                        ...
                        @enderror
                        <small class="text-muted">Pilih permission yang dimiliki role ini</small>
                    </div>
...
function resetForm() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            document.getElementById('level').value = '{{ $role->level ?? 1 }}';
            document.getElementById('is_active').checked = {{ !$role->deleted_at ? 'true' : 'false' }};

            // Reset permissions
            @foreach($permissions as $permission)
                document.getElementById('permission_{{ $permission->id }}').checked = {{ in_array($permission->id, $role->permissions->pluck('id')->toArray()) ? 'true' : 'false' }};
            @endforeach
        }
    });
}
