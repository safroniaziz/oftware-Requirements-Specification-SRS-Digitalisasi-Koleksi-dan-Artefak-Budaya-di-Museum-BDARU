                    ...
                    <table class="table table-hover table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                        <thead>
                            <tr class="fw-bold text-muted bg-light-primary">
                                ...
                                <th class="min-w-125px">
                                    <span class="text-dark fw-bold">QR Code</span>
                                </th>
                                <th class="min-w-100px">
                                    <span class="text-dark fw-bold">Preview QR</span>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr class="border-bottom">
                                ...
                                <td class="text-center">
                                    @if($collection->qrCodes->count() > 0)
                                        ...
                                        <button type="button" class="btn btn-icon btn-sm btn-light-warning" onclick="resetQrCode({{ $collection->id }})" data-bs-toggle="tooltip" title="Reset QR Code">
                                ...
                                <td class="text-center">
                                    @if($collection->qrCodes->count() > 0)
                                        <button type="button" class="btn btn-icon btn-sm btn-light-info" onclick="previewQrCode({{ $collection->id }})" data-bs-toggle="tooltip" title="Preview QR Code">
                                            <i class="fas fa-qrcode"></i>
                                        </button>
                                    @else
...
function showQrCodesModal(qrCodes) {
    ...
    qrCodes.forEach((qr, index) => {
        ...
        const latestBadge = isLatest ? '<span class="badge badge-success ms-2">Terbaru</span>' : '';

        qrCodesHTML += `
            <div class="col-md-6 mb-4">
                <div class="card border-2 ${isLatest ? 'border-success' : 'border-secondary'}">
                    <div class="card-header text-center">
                        <h6 class="fw-bold mb-0">
                            QR Code ${index + 1}${latestBadge}
                        </h6>
                    </div>
                    <div class="card-body text-center">
                        <img src="${qr.qr_image_url}" alt="QR Code" class="img-fluid border rounded mb-3" style="max-width: 250px;">
                        <div class="text-start">
                            <p class="mb-2"><strong>Kode:</strong> <code>${qr.qr_code}</code></p>
                            <p class="mb-2"><strong>Scan Count:</strong> ${qr.scan_count}</p>
                            <p class="mb-2"><strong>Last Scanned:</strong> ${qr.last_scanned_at || 'Belum pernah di-scan'}</p>
                            <p class="mb-0"><strong>Created:</strong> ${qr.created_at}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    Swal.fire({
        ...
        customClass: {
            container: 'qr-codes-modal'
        }
    });
}

function resetQrCode(collectionId) {
    Swal.fire({
        title: 'Reset QR Code?',
        text: 'Apakah Anda yakin ingin mereset QR Code untuk koleksi ini?',
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#dc3545',
        cancelButtonColor: '#6c757d',
        ...
        if (result.isConfirmed) {
            ...
            .catch(error => {
                console.error('Error:', error);
                Swal.fire('Error!', 'Terjadi kesalahan saat mereset QR Code', 'error');
            });
        }
    });
}

function previewQrCode(collectionId) {
    fetch(`/admin/collections/${collectionId}/qr-code-preview`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                Swal.fire({
                    ...
                    html: `
                        <div class="text-center">
                            <img src="${data.qr_image_url}" alt="QR Code Preview" class="img-fluid border rounded mb-3" style="max-width: 400px;">
                            <p class="text-muted mt-2">Kode: <code>${data.qr_code}</code></p>
                        </div>
                    `,
                    width: '500px',
                    showConfirmButton: true,
        ...
        .catch(error => {
            console.error('Error:', error);
            Swal.fire('Error!', 'Terjadi kesalahan saat memuat QR Code', 'error');
        });
}
