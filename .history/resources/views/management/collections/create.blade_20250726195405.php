...
function testToast() {
    ...
    console.log('Looking for toast-container...');
    ...
    showToast('error', 'Test Error', 'Ini adalah test toast notification error!');
    console.log('Toast created');

    // Check if toast was actually added
    setTimeout(() => {
        const toasts = toastContainer.querySelectorAll('.toast');
        console.log('Toasts in container after creation:', toasts.length);
        console.log('Toast container innerHTML:', toastContainer.innerHTML);

        if (toasts.length > 0) {
            const toast = toasts[0];
            console.log('Toast element:', toast);
            console.log('Toast computed style:', window.getComputedStyle(toast));
            console.log('Toast position:', toast.offsetTop, toast.offsetLeft);
            console.log('Toast dimensions:', toast.offsetWidth, toast.offsetHeight);
        }
    }, 100);
}
</script>
