                        ...
                        <div class="col-lg-8">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="mb-4">
                                        <label for="video_path" class="form-label fw-bold">Video Berita</label>
                                        <input type="file" class="form-control @error('video_path') is-invalid @enderror"
                                               id="video_path" name="video_path" accept="video/*">
                                        @error('video_path')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                        <div class="form-text">Upload video untuk berita (opsional)</div>

                                        @if($news->video_path)
                                            <div class="mt-3">
                                                <label class="form-label fw-bold">Video Saat Ini</label>
                                                <div class="d-flex align-items-center">
                                                    <video controls style="max-width: 200px; max-height: 150px;" class="me-3">
                                                        <source src="{{ asset('storage/' . $news->video_path) }}" type="video/mp4">
                                                        Browser Anda tidak mendukung tag video.
                                                    </video>
                                                    <div>
                                                        <small class="text-muted d-block">Video saat ini akan tetap digunakan jika tidak ada upload baru</small>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                        ...
                        <div class="col-lg-4">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="mb-4">
                                        <label for="type" class="form-label fw-bold">Tipe Berita</label>
                                        <select class="form-select @error('type') is-invalid @enderror"
                                                id="type" name="type">
                                            <option value="news" {{ old('type', $news->type) == 'news' ? 'selected' : '' }}>Berita</option>
                                            <option value="article" {{ old('type', $news->type) == 'article' ? 'selected' : '' }}>Artikel</option>
                                            <option value="announcement" {{ old('type', $news->type) == 'announcement' ? 'selected' : '' }}>Pengumuman</option>
                                        </select>
                                        @error('type')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                        <div class="form-text">Tipe konten berita</div>
                                    </div>

                                    <div class="mb-4">
                                        <label for="location" class="form-label fw-bold">Lokasi</label>
                                        <input type="text" class="form-control @error('location') is-invalid @enderror"
                                               id="location" name="location" value="{{ old('location', $news->location) }}"
                                               placeholder="Masukkan lokasi">
                                        @error('location')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                        <div class="form-text">Lokasi terkait berita (opsional)</div>
                                    </div>
