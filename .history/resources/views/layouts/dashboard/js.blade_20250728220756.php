    ...
    <script>
        // Prevent autofocus conflicts
        (function() {
            // Remove any existing autofocus attributes
            const autofocusElements = document.querySelectorAll('[autofocus]');
            autofocusElements.forEach(el => {
                el.removeAttribute('autofocus');
            });

            // Prevent any script from setting autofocus
            const originalSetAttribute = Element.prototype.setAttribute;
            Element.prototype.setAttribute = function(name, value) {
                if (name === 'autofocus') {
                    return; // Block autofocus
                }
                return originalSetAttribute.call(this, name, value);
            };

            console.log('Autofocus prevention enabled');
        })();

        // Clean up Vue.js instances immediately
        (function() {
            // Remove any existing Vue app containers
            const vueApps = document.querySelectorAll('#app, [v-cloak], [v-app], [data-page]');
            vueApps.forEach(app => {
            ...
            // Clear any Vue instances from memory
            if (window.Vue && window.Vue.prototype) {
                // Destroy any existing Vue instances
                const vueInstances = document.querySelectorAll('[data-v-]');
                vueInstances.forEach(instance => {
                    if (instance.__vue__) {
                        instance.__vue__.$destroy();
                    }
                });
            }

            // Clear any Inertia data
            if (window.Inertia) {
                window.Inertia = null;
            }

            // Clear any Swiper instances
            if (window.Swiper) {
                const swiperInstances = document.querySelectorAll('.swiper-container, .swiper');
