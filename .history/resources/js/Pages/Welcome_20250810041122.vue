...
// QR Code methods
const showQrCode = (collection) => {
    if (!collection.latest_qr_code) {
        alert('QR Code belum tersedia untuk koleksi ini.');
        return;
    }

    // Show QR code in SweetAlert2
    Swal.fire({
        title: `QR Code - ${collection.name}`,
        html: `
            <div class="text-center">
                <img src="${collection.latest_qr_code.qr_image_url}" alt="QR Code" class="mx-auto mb-4" style="max-width: 200px;" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                <div style="display: none; color: red; margin-bottom: 1rem;">Gambar QR Code tidak dapat dimuat</div>
                <div class="text-sm text-gray-600 mb-2">
                    <strong>Kode QR:</strong> ${collection.latest_qr_code.qr_code}
                </div>
                <div class="text-sm text-gray-600 mb-2">
                    <strong>Total Scan:</strong> ${collection.latest_qr_code.scan_count || 0} kali
                </div>
                <div class="text-sm text-gray-600">
                    <strong>Terakhir di-scan:</strong> ${collection.latest_qr_code.last_scanned_at ? new Date(collection.latest_qr_code.last_scanned_at).toLocaleDateString('id-ID') : 'Belum pernah di-scan'}
                </div>
                <div class="mt-4 p-2 bg-blue-50 rounded-lg">
                    <p class="text-xs text-blue-600">
                        Scan QR code ini untuk melihat detail koleksi di mobile device
                    </p>
                </div>
            </div>
        `,
...
function onSwiper(swiper) {
    ...
    console.log('Hero images:', props.heroImages);
}
...
onMounted(() => {
    ...
    totalImages.value = props.heroImages.length;
...
<template>
    ...
    <div class="min-h-screen bg-gray-50">
                ...
                <div class="flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
                    ...
                    <div class="w-full lg:w-1/2 animate-fade-in-up delay-1000">
                        <div class="relative h-80 lg:h-96 rounded-2xl overflow-hidden shadow-2xl bg-gray-200 group animate-float-subtle">
                            ...
                            >
                                <SwiperSlide
                                    v-for="image in props.heroImages"
                                    :key="image.id"
                                    ...
                                    <div class="relative w-full h-full flex items-center justify-center">
                                        ...
                                        >
                                            <div class="text-center">
                                                ...
                                                <p class="text-emerald-600 text-lg font-medium">{{ image.title }}</p>
                                                <p class="text-emerald-500 text-sm mt-2">{{ image.subtitle }}</p>
                                                <p class="text-emerald-400 text-xs mt-1">{{ image.description }}</p>
                                            </div>
                                        </div>
                                ...
                                <!-- Fallback slide jika tidak ada hero images -->
                                <SwiperSlide v-if="props.heroImages.length === 0" class="relative w-full h-full">
                                    <div class="relative w-full h-full flex items-center justify-center">
                                        <div class="w-full h-full bg-gradient-to-br from-emerald-100 to-teal-100 flex items-center justify-center absolute inset-0">
                                            <div class="text-center">
                                                <svg class="w-24 h-24 text-emerald-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                                </svg>
                                                <p class="text-emerald-600 text-lg font-medium">Museum Digital Balai Adat Rajo Penghulu</p>
                                                <p class="text-emerald-500 text-sm mt-2">Jelajahi kekayaan budaya Indonesia</p>
                                                <p class="text-emerald-400 text-xs mt-1">Admin akan upload gambar slider nanti</p>
                                            </div>
                                        </div>
                                        <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent"></div>
                                        ...
                                        <div class="absolute bottom-4 left-4 right-4 z-10">
                                            <div class="bg-white/95 backdrop-blur-md rounded-xl p-4 border-2 border-emerald-200 transform transition-all duration-500 hover:bg-white shadow-xl">
                                                <h3 class="text-gray-900 font-bold text-lg mb-1">BDARU</h3>
                                                <p class="text-gray-700 text-sm mb-1">Museum Digital Balai Adat Rajo Penghulu</p>
                                                <p class="text-gray-600 text-xs">Jelajahi kekayaan budaya Indonesia melalui teknologi digital yang inovatif</p>
                                            </div>
                                        </div>
                                    </div>
                                </SwiperSlide>
        ...
        <section class="py-16 lg:py-24 bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 relative overflow-hidden">
            ...
            <div class="relative z-10 w-full lg:w-4/5 mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
                    <!-- Tim Member 1 - Tiara -->
                    <div class="group relative bg-white/90 backdrop-blur-md rounded-2xl p-8 border-2 border-emerald-200 shadow-xl hover:shadow-2xl hover:border-emerald-300 transform hover:-translate-y-2 transition-all duration-500" data-aos="fade-up" data-aos-delay="100">
                    ...
                    <!-- Tim Member 2 - Kurnia -->
                    <div class="group relative bg-white/90 backdrop-blur-md rounded-2xl p-8 border-2 border-emerald-200 shadow-xl hover:shadow-2xl hover:border-emerald-300 transform hover:-translate-y-2 transition-all duration-500" data-aos="fade-up" data-aos-delay="200">
                    ...
                    <div class="group relative bg-white/90 backdrop-blur-md rounded-2xl p-8 border-2 border-emerald-200 shadow-xl hover:shadow-2xl hover:border-emerald-300 transform hover:-translate-y-2 transition-all duration-500 md:col-span-2 lg:col-span-1" data-aos="fade-up" data-aos-delay="300">
                ...
                <!-- Project Description -->
                <div class="mt-12 lg:mt-16 bg-white/80 backdrop-blur-md rounded-2xl p-8 border-2 border-emerald-200 shadow-xl" data-aos="fade-up" data-aos-delay="400">
        ...
        <!-- Didukung Oleh Section - Enhanced with Bigger Logos -->
        <section class="py-20 lg:py-28 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 relative overflow-hidden">
            <!-- Enhanced Background Elements -->
            <div class="absolute inset-0">
                <div class="absolute top-0 left-0 w-96 h-96 bg-blue-300/20 rounded-full blur-3xl animate-float-slow"></div>
                <div class="absolute bottom-0 right-0 w-80 h-80 bg-indigo-300/20 rounded-full blur-3xl animate-float-medium"></div>
                <div class="absolute top-1/2 left-1/4 w-64 h-64 bg-slate-300/20 rounded-full blur-3xl animate-float-fast"></div>
            </div>

            <div class="relative z-10 w-full lg:w-4/5 mx-auto px-4 sm:px-6 lg:px-8">
                <!-- Enhanced Header -->
                <div class="text-center mb-16 lg:mb-20" data-aos="fade-up">
                    <div class="inline-flex items-center px-8 py-4 bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-800 rounded-full border-2 border-blue-200 mb-8 group hover:bg-gradient-to-r hover:from-blue-200 hover:to-indigo-200 transition-all duration-300 shadow-xl">
                        <div class="w-3 h-3 bg-blue-500 rounded-full mr-3 animate-pulse"></div>
                        <span class="text-base font-bold">🤝 Didukung Oleh</span>
                    </div>

                    <h2 class="text-4xl md:text-6xl lg:text-7xl font-black mb-8 leading-tight">
                        <span class="bg-gradient-to-r from-blue-700 via-indigo-700 to-purple-700 bg-clip-text text-transparent animate-gradient">
                            Mitra & Pendukung
                        </span>
                    </h2>

                    <p class="text-xl md:text-2xl text-gray-700 font-medium max-w-4xl mx-auto leading-relaxed">
                        Proyek digitalisasi museum ini didukung oleh berbagai institusi terkemuka untuk 
                        <span class="font-bold text-blue-700">melestarikan warisan budaya Indonesia</span>
                    </p>
                </div>

                <!-- Enhanced Logos Grid with Bigger Logos -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-12 mb-12">
                    <!-- Kementerian Pendidikan -->
                    <div class="group bg-white/90 backdrop-blur-md rounded-3xl p-10 lg:p-12 border-2 border-blue-200/50 hover:bg-white hover:border-blue-300 transition-all duration-500 transform hover:-translate-y-3 hover:scale-105 shadow-2xl hover:shadow-3xl text-center" data-aos="fade-up" data-aos-delay="100">
                        <div class="w-32 h-32 lg:w-40 lg:h-40 mx-auto mb-6 rounded-2xl overflow-hidden bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-lg">
                            <img
                                src="/assets/assets/media/kemdiktisaintek.jpeg"
                                alt="Kemdiktisaintek"
                                class="w-full h-full object-contain p-4"
                                @error="$event.target.style.display='none'; $event.target.nextElementSibling.style.display='flex'"
                            />
                            <div class="w-32 h-32 lg:w-40 lg:h-40 bg-gradient-to-br from-blue-500 to-indigo-500 rounded-2xl flex items-center justify-center text-white font-bold text-4xl" style="display: none;">
                                K
                            </div>
                        </div>
                        <h3 class="text-lg lg:text-xl font-bold text-gray-800 mb-3 leading-tight">Kementerian Pendidikan Tinggi, Sains, dan Teknologi Republik Indonesia</h3>
                        <p class="text-sm lg:text-base text-blue-600 font-semibold">Kemendikbud Ristek</p>
                    </div>

                    <!-- DPPM Kemdiktisaintek -->
                    <div class="group bg-white/90 backdrop-blur-md rounded-3xl p-10 lg:p-12 border-2 border-emerald-200/50 hover:bg-white hover:border-emerald-300 transition-all duration-500 transform hover:-translate-y-3 hover:scale-105 shadow-2xl hover:shadow-3xl text-center" data-aos="fade-up" data-aos-delay="200">
                        <div class="w-32 h-32 lg:w-40 lg:h-40 mx-auto mb-6 rounded-2xl overflow-hidden bg-gradient-to-br from-emerald-50 to-teal-50 flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-lg">
                            <img
                                src="/assets/assets/media/kemdikbud.jpeg"
                                alt="Kemdikbud"
                                class="w-full h-full object-contain p-4"
                                @error="$event.target.style.display='none'; $event.target.nextElementSibling.style.display='flex'"
                            />
                            <div class="w-32 h-32 lg:w-40 lg:h-40 bg-gradient-to-br from-emerald-500 to-teal-500 rounded-2xl flex items-center justify-center text-white font-bold text-4xl" style="display: none;">
                                D
                            </div>
                        </div>
                        <h3 class="text-lg lg:text-xl font-bold text-gray-800 mb-3 leading-tight">Direktorat Penelitian dan Pengabdian kepada Masyarakat (DPPM)</h3>
                        <p class="text-sm lg:text-base text-emerald-600 font-semibold">Kemdiktisaintek</p>
                    </div>

                    <!-- BIMA -->
                    <div class="group bg-white/90 backdrop-blur-md rounded-3xl p-10 lg:p-12 border-2 border-teal-200/50 hover:bg-white hover:border-teal-300 transition-all duration-500 transform hover:-translate-y-3 hover:scale-105 shadow-2xl hover:shadow-3xl text-center" data-aos="fade-up" data-aos-delay="300">
                        <div class="w-32 h-32 lg:w-40 lg:h-40 mx-auto mb-6 rounded-2xl overflow-hidden bg-gradient-to-br from-teal-50 to-cyan-50 flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-lg">
                            <img
                                src="/assets/assets/media/bima.jpeg"
                                alt="BIMA"
                                class="w-full h-full object-contain p-4"
                                @error="$event.target.style.display='none'; $event.target.nextElementSibling.style.display='flex'"
                            />
                            <div class="w-32 h-32 lg:w-40 lg:h-40 bg-gradient-to-br from-teal-500 to-cyan-500 rounded-2xl flex items-center justify-center text-white font-bold text-4xl" style="display: none;">
                                B
                            </div>
                        </div>
                        <h3 class="text-lg lg:text-xl font-bold text-gray-800 mb-3 leading-tight">Basis Informasi Penelitian dan Pengabdian kepada Masyarakat</h3>
                        <p class="text-sm lg:text-base text-teal-600 font-semibold">BIMA</p>
                    </div>

                    <!-- Universitas Bengkulu -->
                    <div class="group bg-white/90 backdrop-blur-md rounded-3xl p-10 lg:p-12 border-2 border-purple-200/50 hover:bg-white hover:border-purple-300 transition-all duration-500 transform hover:-translate-y-3 hover:scale-105 shadow-2xl hover:shadow-3xl text-center" data-aos="fade-up" data-aos-delay="400">
                        <div class="w-32 h-32 lg:w-40 lg:h-40 mx-auto mb-6 rounded-2xl overflow-hidden bg-gradient-to-br from-purple-50 to-indigo-50 flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-lg">
                            <img
                                src="/assets/src/images/logo_unib.png"
                                alt="Universitas Bengkulu"
                                class="w-full h-full object-contain p-4"
                                @error="$event.target.style.display='none'; $event.target.nextElementSibling.style.display='flex'"
                            />
                            <div class="w-32 h-32 lg:w-40 lg:h-40 bg-gradient-to-br from-purple-500 to-indigo-500 rounded-2xl flex items-center justify-center text-white font-bold text-4xl" style="display: none;">
                                U
                            </div>
                        </div>
                        <h3 class="text-lg lg:text-xl font-bold text-gray-800 mb-3 leading-tight">Universitas Bengkulu</h3>
                        <p class="text-sm lg:text-base text-purple-600 font-semibold">UNIB</p>
                    </div>
                </div>

                <!-- Enhanced Additional Info -->
                <div class="text-center" data-aos="fade-up" data-aos-delay="500">
                    <div class="bg-white/80 backdrop-blur-md rounded-2xl p-8 border border-gray-200 shadow-xl max-w-4xl mx-auto">
                        <p class="text-base lg:text-lg text-gray-700 leading-relaxed">
                            <span class="font-bold text-blue-700">Kerjasama strategis</span> dengan berbagai institusi pendidikan dan penelitian terkemuka untuk mewujudkan pelestarian budaya Indonesia melalui teknologi digital yang 
                            <span class="font-bold text-indigo-700">inovatif dan berkelanjutan</span>.
                        </p>
                    </div>
                </div>
            </div>
                        ...
                        <div class="p-4 lg:p-6">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center space-x-2">
                                    <!-- QR Code Button -->
                                    <button
                                        v-if="collection.latest_qr_code"
                                        @click="showQrCode(collection)"
                                        class="inline-flex items-center px-2 py-1 bg-blue-50 text-blue-600 text-xs font-medium rounded-lg hover:bg-blue-100 transition-colors duration-200"
                                    >
