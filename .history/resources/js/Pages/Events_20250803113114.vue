...
// Initialize image loading states for all events
onMounted(() => {
    if (props.events.data) {
        props.events.data.forEach(event => {
            if (event.image_url) {
                imageLoadingStates.value[event.id] = true; // Start loading
            } else {
                imageLoadingStates.value[event.id] = false; // No image to load
            }
        });
    }
});
