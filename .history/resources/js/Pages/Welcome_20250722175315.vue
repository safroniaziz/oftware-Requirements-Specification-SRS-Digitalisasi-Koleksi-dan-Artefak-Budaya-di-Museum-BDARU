...
function handleCollectionImageLoad(event, imageId) {
    collectionImages.value[imageId] = true;
}

function handleNewsImageLoad(event, imageId) {
    newsImages.value[imageId] = true;
}

function handleEventImageLoad(event, imageId) {
    eventImages.value[imageId] = true;
}
...
<template>
    ...
    <!-- Loading Screen -->
    <div v-if="isLoading" class="fixed inset-0 bg-gradient-to-br from-emerald-900 via-teal-800 to-cyan-900 z-[9999] flex items-center justify-center">
        <div class="text-center">
            <!-- Animated Logo -->
            <div class="mb-8">
                <div class="w-24 h-24 bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-500 rounded-3xl flex items-center justify-center shadow-2xl mx-auto animate-pulse">
                    <div class="w-16 h-12 bg-white/95 rounded-xl relative">
                        ...
                        <div class="absolute top-1.5 left-1 w-1.5 h-1.5 bg-emerald-500 rounded-sm"></div>
                        <div class="absolute top-1.5 right-1 w-1.5 h-1.5 bg-emerald-500 rounded-sm"></div>
                        <div class="absolute top-4.5 left-1 w-1.5 h-1.5 bg-teal-500 rounded-sm"></div>
                        <div class="absolute top-4.5 right-1 w-1.5 h-1.5 bg-teal-500 rounded-sm"></div>
            ...
            <!-- Loading Text -->
            <h1 class="text-4xl font-black text-white mb-4 animate-fade-in">
                BDARU
            </h1>
            <p class="text-lg text-emerald-200 mb-8 animate-fade-in delay-300">
            ...
            <div class="w-80 mx-auto mb-8">
                <div class="bg-white/20 rounded-full h-3 overflow-hidden">
                    <div
                        class="bg-gradient-to-r from-emerald-400 to-teal-400 h-full rounded-full transition-all duration-500 ease-out"
            ...
            <div class="flex justify-center space-x-2">
                ...
                <div class="w-3 h-3 bg-teal-400 rounded-full animate-bounce delay-100"></div>
                <div class="w-3 h-3 bg-cyan-400 rounded-full animate-bounce delay-200"></div>
            </div>

            <!-- Loading Message -->
            <p class="text-sm text-emerald-300 mt-6 animate-pulse">
                Menyiapkan pengalaman digital yang luar biasa...
            </p>
        </div>
    </div>

    <!-- Main Content -->
    <div v-show="showContent" class="min-h-screen bg-gray-50 animate-fade-in">
