...
const submit = () => {
    form.post(route('login'), {
        ...
        onSuccess: (response) => {
            // Check if the response is a redirect to dashboard
            if (response && response.url && (response.url.includes('/dashboard') || response.url.includes('/pengelola/dashboard'))) {
                // Force a full page redirect for dashboard routes
