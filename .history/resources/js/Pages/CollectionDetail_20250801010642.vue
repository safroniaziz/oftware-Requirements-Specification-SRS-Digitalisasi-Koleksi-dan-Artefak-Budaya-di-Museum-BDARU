...
// Debug: Log collection data on mount
onMounted(() => {
    console.log('Collection data:', props.collection);
    console.log('Gallery images:', props.collection.gallery_images);
    console.log('Gallery images count:', props.collection.gallery_images?.length);
...
const submitRating = async () => {
    ...
    try {
        ...
        const response = await fetch(`/collections/${props.collection.id}/ratings`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
...
<template>
    <Head :title="`${collection.name} - ${props.museumSettings?.museum_name || 'BDARU Museum Digital Indonesia'}`" />
        ...
        <div class="w-full lg:w-4/5 mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-wrap justify-center border-b border-gray-200 mb-12">
                ...
                >
                    <span class="flex items-center space-x-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.363 1.118l1.07 3.292c.3.922-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.783.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                        </svg>
            ...
            <div class="min-h-[400px]">
                ...
                <div class="bg-yellow-100 p-4 rounded-lg mb-4">
                    <p class="text-sm text-gray-700">
                        <strong>Debug Tab Content:</strong><br>
                        Active Tab: {{ activeTab }}<br>
                        Is Overview: {{ activeTab === 'overview' }}<br>
                        Is Gallery: {{ activeTab === 'gallery' }}<br>
                        Is Ratings: {{ activeTab === 'ratings' }}
                    </p>
                </div>
                                ...
                                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <div class="flex items-center">
                                            ...
                                            <span class="text-gray-900 font-semibold ml-1">{{ collection.average_rating ? collection.average_rating.toFixed(1) : '0.0' }}</span>
                                        </div>
                        ...
                        <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border border-purple-200">
                            ...
                            <div class="bg-white/50 rounded-lg p-3 border border-purple-100">
                                <p class="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{{ collection.nilai_edukatif || 'Memberikan pembelajaran tentang teknik dan filosofi budaya tradisional' }}</p>
                                                                ...
                                                                <!-- Gallery Tab -->
                <div v-show="activeTab === 'gallery'" class="space-y-8" data-aos="fade-up">
                    <div class="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
                        <h3 class="text-2xl font-bold text-gray-900 mb-6">Galeri Detail</h3>

                        <!-- Simple Test -->
                        <div class="bg-red-100 p-4 rounded-lg mb-6">
                            <h4 class="font-bold text-red-800">GALLERY TAB IS WORKING!</h4>
                            <p class="text-red-700">Active Tab: {{ activeTab }}</p>
                            <p class="text-red-700">Gallery Images Count: {{ collection.gallery_images ? collection.gallery_images.length : 'NULL' }}</p>
                        </div>

                        <!-- Always Show Gallery Images -->
                        <div class="bg-green-100 p-4 rounded-lg mb-6">
                            <h4 class="font-bold text-green-800">Gallery Images (Always Show)</h4>
                            <p class="text-green-700">Count: {{ collection.gallery_images ? collection.gallery_images.length : 'NULL' }}</p>
                            <div class="mt-2">
                                <div v-for="(img, index) in collection.gallery_images" :key="img.id" class="text-sm text-green-600">
                        ...
                        <!-- Main Image -->
                        <div v-if="collection.image_url" class="mb-8">
                            <h4 class="text-lg font-semibold text-gray-800 mb-4">Gambar Utama</h4>
                        ...
                        <!-- Gallery Images -->
                        <div v-if="collection.gallery_images && collection.gallery_images.length > 0" class="mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                >
                                    <img
                                        :src="getGalleryImageUrl(galleryImage)"
                        ...
                        <!-- Simple Gallery Test -->
                        <div v-if="collection.gallery_images && collection.gallery_images.length > 0" class="bg-green-100 p-4 rounded-lg mb-4">
                            <h4 class="font-bold text-green-800">Gallery Images Found!</h4>
                            <p class="text-green-700">Count: {{ collection.gallery_images.length }}</p>
                            <div class="mt-2">
                                <div v-for="(img, index) in collection.gallery_images" :key="img.id" class="text-sm text-green-600">
                                    Image {{ index + 1 }}: {{ img.image_path }} -> {{ img.image_url }}
                                </div>
                            </div>
                        </div>

                        <!-- No Gallery Images Message -->
                        <div v-else-if="!collection.image_url && (!collection.gallery_images || collection.gallery_images.length === 0)" class="text-center py-12">
                        ...
                        <div v-else-if="collection.image_url && (!collection.gallery_images || collection.gallery_images.length === 0)" class="text-center py-8">
                            <div class="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4">
                                <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                            </div>
                            <h4 class="text-lg font-semibold text-gray-900 mb-2">Hanya Gambar Utama</h4>
                            <p class="text-gray-600">Saat ini hanya gambar utama yang tersedia untuk koleksi ini.</p>
                        </div>
                    </div>
                </div>

                <!-- Ratings Tab -->
                <div v-if="activeTab === 'ratings'" class="space-y-8" data-aos="fade-up">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
