                        ...
                        <div class="relative h-96 lg:h-[500px] rounded-3xl overflow-hidden shadow-2xl bg-gray-200">
                            ...
                            <div class="absolute top-2 left-2 z-50 bg-black/80 text-white px-2 py-1 rounded text-xs">
                                Images: {{ heroImages.length }}
                                                        ...
                                                        <!-- Debug container -->
                            <div class="relative w-full h-full bg-gray-300 border-4 border-red-500">
                                <!-- Debug info -->
                                <div class="absolute top-2 left-2 z-50 bg-red-500 text-white px-2 py-1 rounded text-xs">
                                    Images: {{ heroImages.length }} | Height: {{ $el?.offsetHeight }}px
                                </div>

                                <!-- Test image 1: Simple img tag -->
                                <div class="absolute top-10 left-2 w-32 h-32 bg-yellow-300 border-2 border-blue-500">
                                    <img
                                        :src="heroImages[0]?.src"
                                        alt="Test 1"
                                        class="w-full h-full object-cover"
                                        @load="() => console.log('Test image 1 loaded')"
                                        @error="() => console.log('Test image 1 error')"
                                    />
                                </div>

                                <!-- Test image 2: Background image -->
                                <div
                                    class="absolute top-10 left-40 w-32 h-32 bg-green-300 border-2 border-blue-500"
                                    :style="{ backgroundImage: `url(${heroImages[0]?.src})`, backgroundSize: 'cover' }"
                                >
                                </div>

                                <!-- Test image 3: Direct src -->
                                <img
                                    :src="heroImages[0]?.src"
                                    alt="Test 3"
                                    class="absolute top-10 right-2 w-32 h-32 border-2 border-purple-500"
                                    @load="() => console.log('Test image 3 loaded')"
                                    @error="() => console.log('Test image 3 error')"
                                />

                                <!-- Original image -->
                                <div v-if="heroImages.length > 0" class="absolute inset-0">
                                    <img
                                        :src="heroImages[0].src"
