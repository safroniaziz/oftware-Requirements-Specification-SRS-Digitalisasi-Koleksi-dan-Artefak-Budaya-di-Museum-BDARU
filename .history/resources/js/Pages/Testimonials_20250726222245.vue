...
// Form data
const form = useForm({
    name: isAuthenticated.value ? props.auth.user.name : '',
    profession: isAuthenticated.value ? props.auth.user.profession : '',
    content: '',
    rating: 0,
    email: isAuthenticated.value ? props.auth.user.email : '',
    phone: isAuthenticated.value ? props.auth.user.phone : ''
});

// Submit testimonial function
function submitTestimonial() {
    // Validasi client-side
    if (!form.name && !isAuthenticated.value) {
        alert('Nama harus diisi');
        return;
    }

    if (!form.content || form.content.length < 10) {
        alert('Testimoni harus diisi minimal 10 karakter');
        return;
    }
            ...
            <div class="max-w-4xl mx-auto">
                ...
                <div class="bg-white rounded-3xl shadow-2xl p-8 md:p-12 border border-gray-100">
                    <form @submit.prevent="submitTestimonial" class="space-y-8">
                        ...
                        <!-- User Info Display - Only show for authenticated users -->
                        <div v-if="isAuthenticated" class="bg-emerald-50 border border-emerald-200 rounded-xl p-6">
                            <h3 class="text-lg font-bold text-emerald-800 mb-4">Informasi Anda</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                <div>
                                    <span class="font-semibold text-emerald-700">Nama:</span>
                                    <span class="ml-2 text-emerald-600">{{ form.name }}</span>
                                </div>
                                <div>
                                    <span class="font-semibold text-emerald-700">Profesi:</span>
                                    <span class="ml-2 text-emerald-600">{{ form.profession || 'Belum diisi' }}</span>
                                </div>
                                <div>
                                    <span class="font-semibold text-emerald-700">Email:</span>
                                    <span class="ml-2 text-emerald-600">{{ form.email }}</span>
                                </div>
                                <div>
                                    <span class="font-semibold text-emerald-700">Telepon:</span>
                                    <span class="ml-2 text-emerald-600">{{ form.phone || 'Belum diisi' }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="text-center pt-6">
                            <button
                                ...
                                :disabled="!form.name || !form.content || form.content.length < 10 || form.rating === 0"
                            >
                                <svg class="w-6 h-6 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
