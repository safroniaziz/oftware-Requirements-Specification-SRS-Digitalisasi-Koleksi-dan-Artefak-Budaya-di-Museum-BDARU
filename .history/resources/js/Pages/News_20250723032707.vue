        ...
        <section class="py-16 bg-gray-50">
            <div class="w-full lg:w-4/5 mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center justify-between mb-8">
                    <div class="text-gray-600">
                        Menampilkan <span class="font-semibold text-emerald-600">{{ news.data.length }}</span> dari <span class="font-semibold">{{ news.total }}</span> berita
                    </div>
            ...
            <div v-if="activeTab === 'featured' && viewMode === 'grid'" class="space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div
                        v-for="article in (props.featuredNews || [])"
                        :key="article.id"
            ...
            <div v-if="viewMode === 'list' && (activeTab === 'featured' ? (props.featuredNews || []).length > 0 : (props.news?.data || []).length > 0)" class="space-y-6">
                    <div
                        v-for="article in (activeTab === 'featured' ? (props.featuredNews || []) : (props.news?.data || []))"
                        :key="article.id"
                            ...
                            <div class="flex-1 p-6">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <span class="text-sm text-gray-600">{{ article.category?.name || 'Berita' }}</span>
                                    </div>
                                </div>
                ...
                <!-- Empty State -->
                <div v-if="(activeTab === 'featured' ? (props.featuredNews || []).length === 0 : (props.news?.data || []).length === 0)" class="text-center py-16">
...
const categoriesWithCounts = computed(() => {
    ...
    const categoryList = (props.categories || []).map(category => {
        ...
        const categoryCount = currentData.filter(article =>
            article.category?.slug === (category.slug || category.id)
        ).length;
...
const filteredNews = computed(() => {
    // Use all news data, not just paginated data
    let filtered = props.news?.data || [];

    // If no data in props.news.data, try to use featuredNews or recentNews
    if (filtered.length === 0) {
        if (activeTab.value === 'featured') {
            filtered = props.featuredNews || [];
        } else {
            filtered = props.recentNews || [];
        }
    }
    ...
    if (selectedCategory.value !== 'all') {
        filtered = filtered.filter(article => article.category?.slug === selectedCategory.value);
    }
