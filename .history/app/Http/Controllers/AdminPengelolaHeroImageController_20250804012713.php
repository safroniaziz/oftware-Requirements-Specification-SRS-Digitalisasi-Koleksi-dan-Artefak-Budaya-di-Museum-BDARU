...
class AdminPengelolaHeroImageController extends Controller
{
    ...
    {
        $heroImages = HeroImage::orderBy('sort_order', 'asc')
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->only([
            'title', 'subtitle', 'description', 'alt_text', 'sort_order', 'is_active'
        ]);
        ...
        if ($request->hasFile('image')) {
            ...
            // Resize image to 1200x800 (3:2 aspect ratio) for hero slider - optimized for web
            $resizedImage = Image::make($image)->resize(1200, 800, function ($constraint) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            // Resize image to 1920x1080 (16:9 aspect ratio) for hero slider
            $resizedImage = Image::make($image)->resize(1920, 1080, function ($constraint) {
                $constraint->aspectRatio();
                $constraint->upsize();
            })->encode('jpg', 85);
