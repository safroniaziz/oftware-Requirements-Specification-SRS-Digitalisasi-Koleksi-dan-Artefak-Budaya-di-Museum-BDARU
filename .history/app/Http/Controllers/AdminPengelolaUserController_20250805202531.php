...
class AdminPengelolaUserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        Log::info('User creation request data', [
            'email_verified_at_raw' => $request->email_verified_at,
            'email_verified_at_boolean' => (bool) $request->email_verified_at,
            'email_verified_at_processed' => $request->has('email_verified_at') ? now() : null,
        ]);
        ...
        if ($request->has('roles')) {
            // Get the roles by ID and assign with explicit guard
            $roleModels = Role::whereIn('id', $request->roles)->get();
            foreach ($roleModels as $role) {
                $user->assignRole($role->name, 'web');
            }
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Get the roles by ID and sync with explicit guard
        $roleModels = Role::whereIn('id', $request->roles)->get();
