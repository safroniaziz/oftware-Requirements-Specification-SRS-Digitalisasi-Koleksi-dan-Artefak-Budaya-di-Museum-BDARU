...
class CollectionController extends Controller
{
    ...
    {
        $query = Collection::with(['category', 'latestQrCode'])->whereNull('deleted_at');
        ...
        // Debug eager loading
        \Log::info("Collections count: " . $collections->count());
        foreach ($collections as $collection) {
            \Log::info("Collection {$collection->name} - Has latestQrCode: " . ($collection->latestQrCode ? 'YES' : 'NO'));
            if ($collection->latestQrCode) {
                \Log::info("  QR Code: {$collection->latestQrCode->qr_code}");
                \Log::info("  QR Image Path: {$collection->latestQrCode->qr_image_path}");
            }
        }
        ...
        return Inertia::render('Collections', [
            'collections' => $collections->through(function ($collection) {
                ...
                if ($collection->latestQrCode && $collection->latestQrCode->qr_image_path) {
                    $qrImageUrl = asset('storage/' . $collection->latestQrCode->qr_image_path);

                    // Debug QR code data
                    \Log::info("QR Code for collection {$collection->name}:", [
                        'qr_code' => $collection->latestQrCode->qr_code,
                        'qr_image_path' => $collection->latestQrCode->qr_image_path,
                    ...
                    \Log::info("No QR code for collection {$collection->name}");
                }
