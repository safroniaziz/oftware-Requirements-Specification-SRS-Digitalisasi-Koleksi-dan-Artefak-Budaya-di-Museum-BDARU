...
class NewsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Transform news data to include image URLs
        ...
        // Transform featured news data
        $featuredNews->transform(function ($item) {
            $item->image_url = $item->image_path ? asset('storage/' . $item->image_path) : null;
            return $item;
        });
        ...
        // Transform recent news data
        $recentNews->transform(function ($item) {
            $item->image_url = $item->image_path ? asset('storage/' . $item->image_path) : null;
            return $item;
        });
    ...
    public function show(News $news)
    {
        ...
        // Add image URL to main news object
        $news->image_url = $news->image_path ? asset('storage/' . $news->image_path) : null;

        // Debug: Log the data being sent
        \Log::info('NewsDetail data', [
            'news_id' => $news->id,
            'news_title' => $news->title,
            'news_image_path' => $news->image_path,
            'news_image_url' => $news->image_url,
            'news_category_id' => $news->news_category_id,
        ]);

        return Inertia::render('NewsDetail', [
            'news' => $news->load(['newsCategory']),
