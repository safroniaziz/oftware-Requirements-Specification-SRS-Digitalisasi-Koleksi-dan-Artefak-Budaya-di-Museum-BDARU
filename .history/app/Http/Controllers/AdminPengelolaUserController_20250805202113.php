...
class AdminPengelolaUserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('roles')) {
            try {
                // Get the roles by ID and assign by name to avoid guard issues
                ...
                Log::info('Assigning roles to user', [
                    ...
                    'role_models' => $roleModels->toArray()
                ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            // Get the roles by ID and sync by name to avoid guard issues
            $roleModels = Role::whereIn('id', $request->roles)->get();
            $roleNames = $roleModels->pluck('name')->toArray();
            ...
            Log::info('Roles synced successfully', [
                'user_id' => $user->id,
                'synced_roles' => $user->roles->pluck('name')->toArray()
            ]);
        } catch (\Exception $e) {
