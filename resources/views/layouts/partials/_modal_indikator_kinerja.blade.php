    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header px-10">
                <h2 class="fw-bold">Tambah Indikator Kinerja</h2>
            </div>
            <div class="modal-body d-flex flex-column scroll-y px-10" style="flex-grow: 1;">
                <form id="kt_modal_form" class="form d-flex flex-column" style="flex-grow: 1;">
                    ...
                    <div class="fv-row mb-5">
                        ...
                        <select name="satuan_standar_id" class="form-control" required>
                            <option disabled selected>-- pilih Sasaran Strategis --</option>
                            @foreach ($satuanStandars as $satuanStandar)
                                <option value="{{ $satuanStandar->id }}">{{ $satuanStandar->sasaran }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="fv-row mb-5">
                        ...
                        <input type="text" name="kode_ikss" class="form-control" required />
                    </div>
    ...
    <script>
        $(document).ready(function () {
            $('#kt_modal_form').on('submit', function (e) {
                e.preventDefault();
                let form = $(this);
                let url = form.attr('action') || "{{ route('indikatorKinerja.store') }}";
