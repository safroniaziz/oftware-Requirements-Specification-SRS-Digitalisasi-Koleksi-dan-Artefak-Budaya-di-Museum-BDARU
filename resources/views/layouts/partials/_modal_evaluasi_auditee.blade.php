    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header px-10">
                <h2 class="fw-bold">Tambah Evaluasi Auditee</h2>
            </div>
            <div class="modal-body d-flex flex-column scroll-y px-10" style="flex-grow: 1;">
                <form id="kt_modal_evaluasi_form" class="form d-flex flex-column" style="flex-grow: 1;">
                    @csrf
...
@push('scripts')
    <script>
        $(document).ready(function () {
            $('#kt_modal_evaluasi_form').on('submit', function (e) {
                e.preventDefault();
                let form = $(this);
                let url = form.attr('action') || "{{ route('evaluasiAuditee.store') }}";
